...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        let mut words = vec![0u128; STACK_SIZE / core::mem::size_of::<u128>()];
        let stack_top = words.as_mut_ptr() as usize + STACK_SIZE;
        let len = words.len() * core::mem::size_of::<u128>();
        let cap = words.capacity() * core::mem::size_of::<u128>();
        let ptr = words.as_mut_ptr() as *mut u8;
        core::mem::forget(words);
        let stack = unsafe { Vec::from_raw_parts(ptr, len, cap) };
