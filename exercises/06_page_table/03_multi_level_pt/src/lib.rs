...
impl Sv39PageTable {
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                ...
                let pa_base = (pte >> PPN_SHIFT) << 12;
                return TranslateResult::Ok(pa_base + offset);
            }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let root_pte = self.nodes.get(&self.root_ppn).unwrap().entries[idx2];

        let level1_ppn = if root_pte & PTE_V == 0 {
            let next_ppn = self.alloc_node();
            self.nodes.get_mut(&self.root_ppn).unwrap().entries[idx2] =
                (next_ppn << PPN_SHIFT) | PTE_V;
            next_ppn
        } else {
            root_pte >> PPN_SHIFT
        ...
        self.nodes.get_mut(&level1_ppn).unwrap().entries[idx1] =
            ((pa >> 12) << PPN_SHIFT) | flags;
    }
}
